...
void AdvancedSearchButton::on_previous_terms_clicked() {
    ...
    m_searchWidget->setFieldsValues(terms.query,terms.category,terms.author,terms.playlist_id,terms.orderby,terms.time);
}

void AdvancedSearchButton::on_search_requested(const QString & query,const QString & category,const QString & author,const QString & playlist_id,YoutubeOrderBy orderby,YoutubeTime time) {
    emit search_requested(query,category,author,playlist_id,orderby,time);
    last_terms.query = query;
    last_terms.category = category;
    last_terms.author = author;
    last_terms.playlist_id = playlist_id;
